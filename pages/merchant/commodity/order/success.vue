<template>
    <AdminPage title="商品订单">
        <template slot="menu">
            ...
            <a href="javascript:;" class="active">已结算</a>
        </template>
        ...
        >
            <el-table-column type="expand" v-slot="{ row }" width="50"  fixed="left">
                <ul class="table-expand">
                    ...
                    <template v-if="row.ticket">
                        ...
                        <li><label>开票日期</label>{{ $helper.formatDate(row.ticket.date, 'Y年m月d日') }}</li>
                    </template>
                    <li><label>结算时间</label>{{ $helper.formatDate(row.final_at, 'Y年m月d日 H:i') }}</li>
                </ul>
            </el-table-column>
            ...
            <el-table-column label="单价(元)" prop="price" width="90" align="center" />

            <el-table-column label="金额(元)" prop="money" width="90" align="center" />
            
            <el-table-column label="供货商" prop="company.title" width="150" align="center" />

            <el-table-column label="发票编号" prop="ticket.number" width="100"  align="center" />

            <el-table-column label="供货时间" width="150" v-slot="{ row }"  align="center">
                {{ $helper.formatDate(row.supplied_at, 'Y-m-d H:i') }}
            </el-table-column>

            <el-table-column label="结算时间" width="150" v-slot="{ row }"  align="center">
                {{ $helper.formatDate(row.final_at, 'Y-m-d H:i') }}
            </el-table-column>
    ...
    export default {
        ...
        async asyncData(ctx) {
            ...
            let searchForm = Object.assign({}, SEARCH_FORM, _.pick(params, [
                'company_id', 'category_id', 'final_at'
            ]))
